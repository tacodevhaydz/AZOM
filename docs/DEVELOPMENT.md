...
- `MozaSerialConnection` — port discovery, background read/write threads (bound to a per-open generation token so a Join-timeout zombie can't re-attach to the next open — see [Threading model](#threading-model)), frame assembly, full 0x7E byte stuffing both directions, classified open-failure surface (`ConnectionFailure`: AccessDenied/PortVanished/… consumed by the UI hint builder). Two write lanes (see [Key Protocol Details](#key-protocol-details)). Registry discovery is primary; the legacy serial probe is an automatic fallback for unclassified ports only, scoped by `MozaProbeTarget` (`BaseAndHub`/`Ab9`/`HubOnly`/`PedalsOnly`/`HandbrakeOnly`/`MBooster`) and hard-disableable via `DisableSerialProbeFallback`.
...
- Diagnostics tab: identity dump, wheel dashboard state, session state, bandwidth (`WriteBudget` window + monotonic peak), wire-error counters, CRC reject counters, serial capture start/stop + ZIP bundle export (`DiagnosticsBundleWriter`: manifest + serial capture + diagnostics text + `MozaLog` snapshot + `upload-log.txt` when a report upload was attempted).
- Bug reports (`SettingsControl.BugReport.cs` + `UI/BugReport/`): the About tab's "Report a problem" card posts that same bundle to the Cloudflare Worker in [`worker/`](../worker/). `BugReportService` records every attempt into `BugReportUploadLog` — request size, HTTP status, the Cloudflare headers (`cf-ray`/`cf-mitigated` mark an edge/WAF block as opposed to a Worker rejection), response body, the full inner-exception chain, and on failure a DNS + system-proxy + plain-`GET /` probe (the Worker answers that with `404 {"error":"not found"}`, so anything else is interception). `DiagnosticsBundleWriter` pulls that log into the bundle on **both** paths, so a user whose uploads are refused can export by hand and the reason ships with the zip; the same record also goes to `MozaLog`, and the failure's short code (`HTTP 403`, `network: timeout`) is appended to the status line. The status line is a read-only `TextBox`, not a `TextBlock` — the ticket reference has to be selectable, and `CopyBugReportRefButton` copies it on its own.
